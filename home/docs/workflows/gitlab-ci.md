...
- Define `stages` explicitly and keep them small: `verify`, `test`, `build`, `stage`, `publish`.
- Use `workflow: rules` to avoid duplicate pipelines.
- Prefer per-job `rules` over `only/except`.
- Use `needs` to reduce critical-path time and `interruptible: true` for cancellable verification jobs.
...
- Treat `GL_CICD_SHARED_PROJ` as a pre-existing GitLab CI/CD variable, not a value set inside the consumer `.gitlab-ci.yml`.
