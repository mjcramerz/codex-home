...
Start with `$CODEX_HOME/plans/workflows/workflow-aptly.md` before executing this workflow.
...
1. Confirm channel names, publish endpoints, snapshot retention, and signing inputs first.
2. Keep queue, state, cache, and secrets paths explicit and non-overlapping.
3. Prefer append-only snapshot publication and deterministic cleanup thresholds.
4. Validate with focused publish-helper or retention smoke tests before widening scope.
