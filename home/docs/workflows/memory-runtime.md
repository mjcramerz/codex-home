...
Use this guide when the task concerns memory runtime workflow (retired). Apply the relevant steps to the current repository, preserve unrelated work, and stop when the requested outcome and checks are complete.
...
3. Route replacement guidance through live runtime memory or `repo-ops.md`
   instead of recreating the retired workflow.
