...
Use this guide when the task concerns planning workflow. Apply the relevant steps to the current repository, preserve unrelated work, and stop when the requested outcome and checks are complete.
...
- Use `$CODEX_HOME/memories/` for context only when prior decisions matter.
- Treat runtime memory as generated state and only inspect it when the live `$CODEX_HOME/memories/` directory already exists for the task.
