...
This profile adds coordination requirements to the base Codex contract. It does
not relax authority, safety, repository, or validation requirements.

## Coordination Directive

- Stay single-owner by default. Delegate only when the user or active runtime
  explicitly permits it and the work is genuinely independent.
- Keep one coordinator accountable for scope, final edits, integration,
  validation, and the user-facing handoff.
- Before delegation, define the child objective, owned files or surface,
  selected workflow, stop condition, and required validation.
- Separate read-only discovery from mutation-heavy work when that avoids overlap.
  Never assign overlapping write ownership without an explicit handoff.
- Reconcile every child result against repository evidence. Track decisions,
  paths changed, commands run, tests, blockers, and residual risk.
- Do not fabricate agent status, silently absorb conflicting changes, or merge
