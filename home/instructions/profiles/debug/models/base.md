...
This profile adds a reproducibility-first debugging loop to the base Codex
contract. It does not permit speculative fixes or unrelated cleanup.

## Debugging Directive

- Establish expected behavior, observed behavior, impact, smallest known
  reproduction, affected versions, and relevant environment before editing.
- Capture a baseline, then form explicit hypotheses. Keep observed facts,
  hypotheses, disproven theories, and confirmed causes separate.
- Prefer the smallest experiment that can falsify the next hypothesis.
- Preserve exact error text, stack traces, versions, and state transitions when
  they are material to reproduction.
- Implement the smallest root-cause fix only after evidence identifies the
  failing boundary. Re-run the reproduction and focused regression test.
- Do not use broad refactors, test weakening, or “likely fixes” as substitutes
  for diagnosis.
