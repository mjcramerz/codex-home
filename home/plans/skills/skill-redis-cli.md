...
Use this plan when you applying or updating the `redis-cli` skill. Fill in the concrete scope, evidence, ordered actions and completion criteria before executing dependent steps. Keep deployment and new test files out of scope unless the task authorizes them.
...
- Read the `redis-cli` skill only when its trigger matches this task and the skill is available.
