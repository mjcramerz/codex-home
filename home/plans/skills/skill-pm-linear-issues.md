# Pm linear issues plan

Use this plan when you applying or updating the `pm-linear-issues` skill. Fill in the concrete scope, evidence, ordered actions and completion criteria before executing dependent steps. Keep deployment and new test files out of scope unless the task authorizes them.

Use this plan when applying or updating the `pm-linear-issues` skill.
...
- Read the `pm-linear-issues` skill only when its trigger matches this task and the skill is available.
- Any referenced scripts, assets, or references in the skill

## Scope

- In: tasks covered by the `pm-linear-issues` skill and its resources.
...
- [ ] Use skill pm-linear-issues and linked resources.
...
- Example objective: "Apply the pm-linear-issues skill to the scoped repository task with explicit validation evidence."
