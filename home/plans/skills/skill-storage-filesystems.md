# Storage filesystems plan

Use this plan when you applying or updating the `storage-filesystems` skill. Fill in the concrete scope, evidence, ordered actions and completion criteria before executing dependent steps. Keep deployment and new test files out of scope unless the task authorizes them.

Use this plan when applying or updating the `storage-filesystems` skill.
...
- In: tasks covered by the `storage-filesystems` skill and its resources.
...
- [ ] Use skill storage-filesystems and linked resources.
...
- Example objective: "Apply the storage-filesystems skill to the scoped repository task with explicit validation evidence."
