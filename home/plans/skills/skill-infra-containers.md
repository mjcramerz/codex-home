# Infra containers plan

Use this plan when you applying or updating the `infra-containers` skill. Fill in the concrete scope, evidence, ordered actions and completion criteria before executing dependent steps. Keep deployment and new test files out of scope unless the task authorizes them.

Use this plan when applying or updating the `infra-containers` skill.
...
- In: tasks covered by the `infra-containers` skill and its resources.
...
- [ ] Use skill infra-containers and linked resources.
- [ ] Collect required inputs (paths, constraints, desired output).
- [ ] Execute the skill workflow and produce outputs (Docker + Podman, rootless/rootful, dev containers if needed).
...
- Example objective: "Apply the infra-containers skill to the scoped repository task with explicit validation evidence."
