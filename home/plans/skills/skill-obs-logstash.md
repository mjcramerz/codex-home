...
Use this plan when you applying or updating the `obs-logstash` skill. Fill in the concrete scope, evidence, ordered actions and completion criteria before executing dependent steps. Keep deployment and new test files out of scope unless the task authorizes them.

Use this plan when applying or updating the `obs-logstash` skill.
...
- In: tasks covered by the `obs-logstash` skill and its resources.
...
- [ ] Use skill obs-logstash and linked resources.
...
- Example objective: "Apply the obs-logstash skill to the scoped repository task with explicit validation evidence."
