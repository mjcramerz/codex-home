...
Use this plan when you following `$CODEX_HOME/docs/workflows/repo-ops.md`. Fill in the concrete scope, evidence, ordered actions and completion criteria before executing dependent steps. Keep deployment and new test files out of scope unless the task authorizes them.

Use this plan when following `$CODEX_HOME/docs/workflows/repo-ops.md`.
...
- In: steps defined in the `repo-ops` workflow.
...
- [ ] Read `$CODEX_HOME/docs/workflows/repo-ops.md` and related references.
- [ ] Collect required inputs and constraints.
- [ ] Execute the workflow steps in order.
- [ ] Verify module/path invariants for touched automation scripts (layout, path safety, dry-run behavior).
...
- Example objective: "Execute the repo-ops workflow for the current repository scope."
