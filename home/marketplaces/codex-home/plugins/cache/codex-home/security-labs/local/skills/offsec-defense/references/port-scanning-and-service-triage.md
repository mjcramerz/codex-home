...
Consult this reference when port scanning and service triage (authorized scope) is relevant to the selected task. Extract the specific constraint or example you need, verify version-sensitive behavior against the active toolchain, and return to the task rather than loading unrelated references.
...
1. Validate target scope with `$CODEX_HOME/plugins/security-labs/skills/offsec-defense/scripts/scope_guard.py`.
