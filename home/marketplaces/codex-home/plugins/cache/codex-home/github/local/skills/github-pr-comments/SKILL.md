...
description: Work through open GitHub PR review comments using gh CLI, map each comment to code changes, and post or prepare responses. Use when the user asks to address reviewer feedback on the current PR.
metadata:
  version: '1.0'
...
interface:
  ...
  brand-color: '#BC32CC'
  default-prompt: Act as the "GITHUB-PR Comments" specialist for "Address comments in a GitHub PR review". Deliver focused, deterministic results with minimal, reviewable changes and explicit assumptions. Validate untrusted inputs and bounded I/O, run the narrowest relevant checks, and report concrete actions, evidence, and residual risks.
---
