...
Consult this reference when pack config format (json) is relevant to the selected task. Extract the specific constraint or example you need, verify version-sensitive behavior against the active toolchain, and return to the task rather than loading unrelated references.

Use this file format with `$CODEX_HOME/plugins/health-planning/skills/health-adhd-cbt/scripts/build_pack.py` from the skill root.
...
- `css` (optional): CSS path relative to the skill root `$CODEX_HOME/plugins/health-planning/skills/health-adhd-cbt/assets/` directory (must not escape root) or an absolute path.
- `pages` (required): array of page entries (see below).

## Page entry fields

- `template` (required): template filename or path. If no extension, `.html` is added.
  Relative paths are resolved under `$CODEX_HOME/plugins/health-planning/skills/health-adhd-cbt/assets/templates/` and may not escape that directory.
