...
```bash
$CODEX_HOME/plugins/cache/codex-home/huggingface/local/skills/huggingface-tool-builder/references/baseline_hf_api.sh 25 \
  | jq -r '.[].id' \
  | $CODEX_HOME/plugins/cache/codex-home/huggingface/local/skills/huggingface-tool-builder/references/hf_enrich_models.sh \
  | jq -s 'sort_by(.downloads) | reverse | .[:10]'

$CODEX_HOME/plugins/cache/codex-home/huggingface/local/skills/huggingface-tool-builder/references/baseline_hf_api.sh 50 \
  | jq '[.[] | {id, downloads}] | sort_by(.downloads) | reverse | .[:10]'

printf '%s\n' openai/gpt-oss-120b meta-llama/Meta-Llama-3.1-8B \
  | $CODEX_HOME/plugins/cache/codex-home/huggingface/local/skills/huggingface-tool-builder/references/hf_model_card_frontmatter.sh \
