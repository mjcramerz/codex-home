...
Consult this reference when instructioning best practices (tts) is relevant to the selected task. Extract the specific constraint or example you need, verify version-sensitive behavior against the active toolchain, and return to the task rather than loading unrelated references.
...
For copy/paste instruction templates, see `$CODEX_HOME/plugins/ai-media/skills/speech/references/sample-prompts.md`. This file focuses on principles, structure, and iteration patterns.
