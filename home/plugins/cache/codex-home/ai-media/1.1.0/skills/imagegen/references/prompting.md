...
Consult this reference when prompting best practices (gpt-image-1.5) is relevant to the selected task. Extract the specific constraint or example you need, verify version-sensitive behavior against the active toolchain, and return to the task rather than loading unrelated references.
...
For copy/paste prompt specs (examples only), see `$CODEX_HOME/plugins/ai-media/skills/imagegen/references/sample-prompts.md`. This file focuses on principles, structure, and iteration patterns.
