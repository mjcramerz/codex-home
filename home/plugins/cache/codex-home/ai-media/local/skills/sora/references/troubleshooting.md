...
- Fix: match size to model; use only "4", "8", or "12" seconds (see `$CODEX_HOME/plugins/cache/codex-home/ai-media/local/skills/sora/references/video-api.md`).
- If you see `invalid_type` for seconds, update `$CODEX_HOME/plugins/cache/codex-home/ai-media/local/skills/sora/scripts/sora.py` or pass a string value for `--seconds`.
...
- Fix: use `--prompt-file prompt.txt` (see `$CODEX_HOME/plugins/cache/codex-home/ai-media/local/skills/sora/references/cli.md` for an example).
