...
```bash
export WEB_GAME_CLIENT="scripts/web_game_playwright_client.js"
export WEB_GAME_ACTIONS="references/action_payloads.json"
```
...
7. **Run the Playwright test script.** Run `$WEB_GAME_CLIENT` after each meaningful change; do not invent a new client unless required.
...
- `$WEB_GAME_CLIENT` (bundled script: `scripts/web_game_playwright_client.js`) — Playwright-based action loop with virtual-time stepping, screenshot capture, and console error buffering. Pass an action burst via `--actions-file`, `--actions-json`, or `--click`.

## References

- `$WEB_GAME_ACTIONS` (bundled file: `references/action_payloads.json`) — example action payloads (keyboard + mouse, per-frame capture). Use these to build your burst.
