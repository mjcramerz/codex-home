...
Consult this reference when troubleshooting is relevant to the selected task. Extract the specific constraint or example you need, verify version-sensitive behavior against the active toolchain, and return to the task rather than loading unrelated references.

## Job fails with size or seconds errors

- Cause: size not supported by model, or seconds not in 4/8/12.
- Fix: match size to model; use only "4", "8", or "12" seconds (see `$CODEX_HOME/plugins/ai-media/skills/sora/references/video-api.md`).
- If you see `invalid_type` for seconds, update `$CODEX_HOME/plugins/ai-media/skills/sora/scripts/sora.py` or pass a string value for `--seconds`.
...
- Fix: use `--prompt-file prompt.txt` (see `$CODEX_HOME/plugins/ai-media/skills/sora/references/cli.md` for an example).
