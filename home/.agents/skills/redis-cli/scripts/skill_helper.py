#!/usr/bin/env python3
"""Utility helper for the redis-cli skill.
...
def command_catalog_path() -> Path:
    return skill_root() / 'references' / 'command-catalog.md'
...
def has_filesystem_dependency(openai_yaml_path: Path) -> bool:
    ...
    for tool in tools:
        ...
        if dep_type == 'mcp' and dep_value == 'filesystem':
            return True
    return False
...
def summary_payload() -> dict[str, Any]:
    return {
        ...
        'command_catalog_file': str(command_catalog_path()),
...
def cmd_validate() -> int:
    ...
    if not command_catalog.is_file():
        errors.append(f"missing command catalog: {command_catalog}")
